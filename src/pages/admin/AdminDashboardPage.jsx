...
export default function AdminDashboardPage() {
  ...
  // Calculate dynamic stats from mock state
  const stats = useMemo(() => {
    const totalDownloads = games.reduce((acc, g) => acc + (g.downloadCount || 0), 0)
    ...
    const publishedCount = games.filter((g) => g.status !== 'Draft').length
    ...
    return {
      ...
      publishedCount,
      draftCount,
      totalUsers: MOCK_USERS_LIST.length + 128, // mock active user count
      totalDownloads: formattedTotalDownloads,
      downloadsToday: '1,420',
    }
  }, [games])
  ...
  // Mock activity logs
  const activities = [
    { id: 1, type: 'create', message: 'New game "Shattered Pixel Dungeon" published', time: '10m ago', icon: PlusCircle, color: 'text-emerald-400 bg-emerald-500/10' },
    { id: 2, type: 'update', message: 'System requirements updated for Trackmania', time: '1h ago', icon: Edit, color: 'text-primary-400 bg-primary-500/10' },
    { id: 3, type: 'user', message: 'New user "CyberValkyrie" registered', time: '3h ago', icon: Users, color: 'text-indigo-400 bg-indigo-500/10' },
    { id: 4, type: 'system', message: 'Automated catalog sync completed', time: '6h ago', icon: CheckCircle2, color: 'text-emerald-400 bg-emerald-500/10' },
  ]
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-2xl bg-gradient-to-r from-surface-800 via-surface-800/90 to-primary-950/40 border border-surface-700/60 shadow-xl relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-sm text-gray-400 mt-1">
            Real-time overview of GameVault catalog, mock metrics, and distribution operations.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-surface-800/90 border border-surface-700/60 shadow-lg relative overflow-hidden">
          ...
          <div className="text-3xl font-black text-white mb-1">{stats.totalGames}</div>
          <div className="flex items-center gap-2 text-xs text-gray-400">
            <span className="text-emerald-400 font-medium">{stats.publishedCount} Published</span>
            <span>•</span>
            <span className="text-amber-400 font-medium">{stats.draftCount} Drafts</span>
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-surface-800/90 border border-surface-700/60 shadow-lg relative overflow-hidden">
          ...
          <div className="text-3xl font-black text-white mb-1">{stats.totalUsers}</div>
        ...
        <div className="p-5 rounded-2xl bg-surface-800/90 border border-surface-700/60 shadow-lg relative overflow-hidden">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold uppercase tracking-wider text-gray-400">Total Downloads</span>
            <div className="w-10 h-10 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 flex items-center justify-center">
              <DownloadCloud className="w-5 h-5" />
            </div>
          </div>
          <div className="text-3xl font-black text-white mb-1">{stats.totalDownloads}</div>
        ...
        <div className="p-5 rounded-2xl bg-surface-800/90 border border-surface-700/60 shadow-lg relative overflow-hidden">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold uppercase tracking-wider text-gray-400">Downloads Today</span>
            <div className="w-10 h-10 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 flex items-center justify-center">
              <Clock className="w-5 h-5" />
            </div>
          </div>
          <div className="text-3xl font-black text-white mb-1">{stats.downloadsToday}</div>
          <p className="text-xs text-emerald-400 flex items-center gap-1 font-medium">
        ...
        <div className="lg:col-span-2 p-6 rounded-2xl bg-surface-800/90 border border-surface-700/60 shadow-lg">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">Monthly mock distribution volume</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-2xl bg-surface-800/90 border border-surface-700/60 shadow-lg">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/games" className="text-xs font-semibold text-primary-400 hover:text-primary-300 transition-colors">
              View All &rarr;
            </Link>
          </div>
          <div className="divide-y divide-surface-700/50">
            {recentGames.map((game) => (
              <div key={game.id} className="py-3 flex items-center justify-between gap-3 first:pt-0 last:pb-0">
        ...
        <div className="p-6 rounded-2xl bg-surface-800/90 border border-surface-700/60 shadow-lg">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/downloads" className="text-xs font-semibold text-primary-400 hover:text-primary-300 transition-colors">
              Analytics &rarr;
            </Link>
          </div>
          <div className="divide-y divide-surface-700/50">
            {popularGames.map((game, idx) => (
              <div key={game.id} className="py-3 flex items-center justify-between gap-3 first:pt-0 last:pb-0">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-white truncate">{game.title}</p>
                    <p className="text-xs text-gray-400 truncate">{game.downloads || '0'} downloads</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <Link
